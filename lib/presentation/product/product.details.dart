...
class ProductDetailScreen extends StatefulWidget {
  const ProductDetailScreen({super.key, required this.productsModel});
...
class _ProductDetailScreenState extends State<ProductDetailScreen> {
  ...
  Widget get appbarWidget => CommonAppbar(
    ...
    icon: InkWell(
      onTap: () {
        // We will navigate here, so disable navigation inside fetchCart
  ...
  /* product detail tile */
  Widget get productDetailTile => Column(
    crossAxisAlignment: CrossAxisAlignment.center,
    children: [
      /* title */
      title,
  ...
  Widget get subtitle => Padding(
    ...
    child: Text(
      ...
      textAlign: TextAlign.center,
      style: TextStyle(color: AppColors.primary.withOpacity(0.6), fontSize: 12),
    ),
  );
    ...
    child: Align(
      ...
      child: Row(
        children: [
          ...
          Text(
            "\$${widget.productsModel.discountedPrice}",
            style: TextStyle(
              color: AppColors.primary,
  ...
  Widget get ratingText => Padding(
    ...
    child: InkWell(
      onTap: () {
        Get.to(() => ProductRatingScreen(productsModel: widget.productsModel));
      },
      child: Align(
  ...
  Widget get bottomNavBar => Container(
    ...
    child: Row(
      ...
      children: [
        Expanded(
          child: ElevatedButton(
            onPressed: () {
              ...
              final selectedSize = productcontroller.selectedSizes[productId];
              ...
              try {
                // Update product controller for UI (this will sync to CartController)
                productcontroller.incrementCount(productId);
                productcontroller.addToCart(widget.productsModel);

                // Try fetching API cart if available (cartController already synced)
                // Caller will navigate, so disable navigation inside fetchCart
                fetchcartcontroller.fetchCart(navigateToCart: false);

                // Navigate to cart and ensure header shows
                Get.toNamed('/cart', arguments: {'fromTab': true});
              } catch (e) {
                Get.snackbar(
                  "Error",
        ...
        Expanded(
          child: Obx(() {
            final isInCart = productcontroller.cartProducts.containsKey(
            ...
            return ElevatedButton(
              onPressed: () {
                ...
                final alreadyAdded = productcontroller.cartProducts.containsKey(
                  productId,
                );
                if (alreadyAdded) {
                  Get.snackbar(
